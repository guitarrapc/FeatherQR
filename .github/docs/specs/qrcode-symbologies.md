...
- **The `options` parameter carries a default on every entry point, and could not while the parameter lists existed.** rMQR had `in RmQRCodeGeneratorOptions options = default` from the start because its parameter list versions were deleted before release; on Standard QR and Micro QR a defaulted options parameter would have made `CreateQrCode(text, ecc)` ambiguous between the two sets, so callers wrote `QRCodeGeneratorOptions.Default with { ... }` explicitly until 2.0.0 removed the other set. Sizing never had the problem: `TryGetRequiredBufferSize` was always the only sizing overload, so its options parameter was defaulted from the start. The asymmetry was a consequence of the released surface rather than a preference, which is why removing that surface was enough to end it.
...
With the parameter list overloads gone, whether the `string` convenience overloads are still worth their weight is open: `string` converts implicitly to `ReadOnlySpan<char>` on every target framework, so they add a parameter name (`plainText` rather than `textSpan`) and nothing else. They were kept through the removal because their released counterparts pinned the shape; the decision belongs to the rename workstream that follows in the same major.
...
| 2.0.0 type renames (the `QR` casing rule, `ECCLevel` to `QREccLevel` and friends) and the `string` convenience overloads | Out of the split and out of the removals; their own plan, same major, before `2.0.0` final | That plan |
...
- **A removal is a rename in disguise, and the compiler only finds half of it.** Deleting the parameter list overloads broke 119 call sites the compiler named, and one more it could not: `requestedVersion: -1` meant "automatic", while `QRCodeVersionRange` rejects `-1` on purpose, so the mechanical rewrite produced code that compiled and threw at run time. The test suite caught it; a consumer's would too, which is why the migration guide leads with that trap rather than with the argument-to-property table.
