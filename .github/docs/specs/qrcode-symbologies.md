...
**The `string` convenience overloads went with them.** `string` converts implicitly to `ReadOnlySpan<char>` on all four target frameworks, so `Create("text", ecc)` still binds; what the overloads added was a parameter name (`plainText` rather than `textSpan`) and a second row in every IntelliSense list. They were kept while their released counterparts pinned the shape and dropped in 2.0.0 with them, which also ended the asymmetry where only Standard QR had a `string` destination overload. Every generator is now two `Create` methods, span and span-plus-destination, plus `TryGetRequiredBufferSize`.
...
- **`required` members put `[Obsolete]` on the implicit constructor**, and a metadata-level API listing renders that as if it were a deprecation. `IconData` read as carrying an announced removal in `PublicAPI.approved.txt` and does not; the marker is how the compiler stops a pre-C#-11 consumer from bypassing the required-member check. Read a listing entry against the source before acting on it.
