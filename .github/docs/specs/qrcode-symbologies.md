...
Equality is written by hand over the elements now, and the array is private, which is also what makes the record's shallow copy safe: `Default with { Direction = … }` shares the colours with `Default`, and no member of either instance hands the array out. The guarantee is "immutable through the API", not "unreachable" — `MemoryMarshal.GetReference` takes a writable reference out of any `ReadOnlySpan<T>` with no `unsafe` and no opt-in, so a caller determined to repaint `Default` still can. That is a property of every span-returning API in .NET; what the design removes is the accident. Dropping the record was tried first and rejected — it took `with` away while the three generator options kept it, and the defect was the *public array*, not the record. `IconData` was demoted the same way in the same change and restored for the same reason.
