...
Reference tests: [TryGetRequiredBufferSizeTest](../../../tests/FeatherQR.Tests/Shared/TryGetRequiredBufferSizeTest.cs) (`Try` reports the size the encode then fills, over content × ECC × ECI × UTF-8 BOM; through 1.2.0 this was an agreement check against the released throwing `Get`, re-pointed at the encode before that overload was removed in 2.0.0), [CapacityOverflowGuardTest](../../../tests/FeatherQR.Tests/Shared/CapacityOverflowGuardTest.cs) (a length that wraps the bit count is rejected, argument errors still throw at that length, and the published maxima still fit), [EccBoostTest](../../../tests/FeatherQR.Tests/StandardQr/EccBoostTest.cs) (headroom classes, version invariance, sizing indifference, error parity).
...
| Section 7.10 | Version information (version 7+, two 3×6 patterns) | [ModulePlacer.PlaceVersion](../../../src/FeatherQR/Internals/StandardQR/ModulePlacer.cs), [QRCodeConstants.GetVersionBits](../../../src/FeatherQR/Internals/StandardQR/QRCodeConstants.cs); decoded (up to 3 errors per copy) by [QRImageDecoder.ReadVersionDimension](../../../src/FeatherQR/Internals/StandardQR/QRImageDecoder.cs) to correct the image path's dimension estimate |
...
| Section 7.7.3 / 7.8.2 | Codeword extraction and unmasking (the encoder's placement runs read backwards, four rows a byte; mask from a table of the predicates' 12-row, 6-column period) | [QRMatrixDecoder.ExtractCodewords](../../../src/FeatherQR/Internals/StandardQR/QRMatrixDecoder.Extract.cs); the per-module zigzag walk it is held to is [ExtractCodewordsReference](../../../src/FeatherQR/Internals/StandardQR/QRMatrixDecoder.cs); parity: [QRMatrixDecoderExtractParityTest](../../../tests/FeatherQR.Tests/StandardQr/QRMatrixDecoderExtractParityTest.cs) |
...
| Section 6.3.3 | Finder pattern detection (1:1:3:1:1 ratio scan + cross checks) | [FinderPatternFinder](../../../src/FeatherQR/Internals/ImageDecoders/FinderPatternFinder.cs), scalar row walk plus a bit-identical SIMD mask walk (AVX2 / ARM NEON / portable 128-bit tiers) selected at runtime; parity: [FinderPatternFinderParityTest](../../../tests/FeatherQR.Tests/StandardQr/FinderPatternFinderParityTest.cs). With 256-bit vectors or ARM64 AdvSimd, rows of 32 to 4,095 pixels go to the edge-list kernel instead ([FinderPatternFinder.RowEdges](../../../src/FeatherQR/Internals/ImageDecoders/FinderPatternFinder.RowEdges.cs)): every dark run of the row at once, sixteen windows classified a step (eight on ARM64, whose word comes from the NEON fold and whose verdicts become bits only once a step has a flagged lane), the flagged ones handed in order to the same follow-ups; the three kernels are held to one candidate list by [FinderRowKernelParityTest](../../../tests/FeatherQR.Tests/StandardQr/FinderRowKernelParityTest.cs), and the kernel's two stages to a pixel scan and to the scalar ratio checks by [FinderRowEdgesTest](../../../tests/FeatherQR.Tests/StandardQr/FinderRowEdgesTest.cs). The cross-checks measure their five runs through one walk for the column, the row and the diagonal ([FinderPatternFinder.RunWalk](../../../src/FeatherQR/Internals/ImageDecoders/FinderPatternFinder.RunWalk.cs)), with the loops it replaced kept as its reference; parity: [FinderRunWalkParityTest](../../../tests/FeatherQR.Tests/StandardQr/FinderRunWalkParityTest.cs). The column and row walks measure the centre run first and stop at the first run no accepted cross section could have; the bounds are held against the ratio checks by [FinderRunBoundsTest](../../../tests/FeatherQR.Tests/StandardQr/FinderRunBoundsTest.cs) and the cross-check through both walks by [FinderCrossCheckParityTest](../../../tests/FeatherQR.Tests/StandardQr/FinderCrossCheckParityTest.cs) |
...
| - | Piecewise mesh sampling (version 14+, bilinear over the alignment lattice) | [QRImageDecoder.SampleGridPiecewise](../../../src/FeatherQR/Internals/StandardQR/QRImageDecoder.PiecewiseSampling.cs): an AVX2 tier (net8.0+, eight modules a step), an ARM64 tier (four lanes, two steps a loop body) and a portable column-table tier, all held to the per-module loop `SampleGridPiecewiseScalar` pixel for pixel; parity: [SampleGridPiecewiseParityTest](../../../tests/FeatherQR.Tests/StandardQr/SampleGridPiecewiseParityTest.cs) |
| - | Binarization (Otsu) | [Binarizer](../../../src/FeatherQR/Internals/ImageDecoders/Binarizer.cs), shared across symbologies: the histogram fill has a 256-bit tier that counts the two extreme values in registers, an ARM64 tier that does so without a movemask and spreads dense blocks over four rented sub-histograms, and a scalar walk, the threshold search and the grey levels read the histogram alone, so the inverted retry mirrors the first polarity's histogram instead of counting the negative; parity: [OtsuHistogramParityTest](../../../tests/FeatherQR.Tests/Shared/OtsuHistogramParityTest.cs) (every bin, each tier entered directly), [OtsuInvertedHistogramTest](../../../tests/FeatherQR.Tests/Shared/OtsuInvertedHistogramTest.cs) (mirrored bins against a count of the negative, and a light-on-dark symbol with uneven levels per decoder), [OtsuThresholdParityTest](../../../tests/FeatherQR.Tests/StandardQr/OtsuThresholdParityTest.cs); the grey levels' enable condition, exact at any pixel count: [GreyLevelsEnableTest](../../../tests/FeatherQR.Tests/Shared/GreyLevelsEnableTest.cs) |
| - | Regional binarization when the global threshold reads nothing in either polarity (8 × 8 blocks, each pixel against the mean black point of the 5 × 5 around its block), tried on the positive and then the negative, each from its own pixels and decoded only when it moves a pixel out of the class that polarity's global threshold gave it; an image of only 0 and 255 is not binarized. Shared across symbologies, with a 128-bit tier for its three passes; each attempt is the decoder's own finder search, as for the global threshold (Micro QR and rMQR: a strided scan, then a full sweep when nothing decoded) | [LocalBinarizer](../../../src/FeatherQR/Internals/ImageDecoders/LocalBinarizer.cs), [RegionalRetry](../../../src/FeatherQR/Internals/ImageDecoders/RegionalRetry.cs); parity: [LocalBinarizerParityTest](../../../tests/FeatherQR.Tests/Shared/LocalBinarizerParityTest.cs), retry: [RegionalRetryTest](../../../tests/FeatherQR.Tests/Shared/RegionalRetryTest.cs), decode: [UnevenLightingDecodeTest](../../../tests/FeatherQR.Tests/Shared/UnevenLightingDecodeTest.cs) |
| - | Module-size measurement (whole-pixel runs, then sub-pixel edges on an image with grey levels), dimension estimation | [QRImageDecoder](../../../src/FeatherQR/Internals/StandardQR/QRImageDecoder.cs) |
| - | The finders' frame: the plane the three finder centres and the module sizes at the ends of the two finder lines fix, where the bottom-right alignment pattern is searched and the timing patterns are matched, and the grid sampled when nothing anchors the fourth corner and the parallelogram failed; parity with a plane: [FinderFrameTest](../../../tests/FeatherQR.Tests/StandardQr/FinderFrameTest.cs) | [QRImageDecoder.FinderFrame](../../../src/FeatherQR/Internals/StandardQR/QRImageDecoder.FinderFrame.cs) |

Reference tests: [QRCodeDecoderRoundTripTest](../../../tests/FeatherQR.Tests/StandardQr/QRCodeDecoderRoundTripTest.cs) (encode→decode round-trips, error injection), [EccBinaryDecoderUnitTest](../../../tests/FeatherQR.Tests/Shared/EccBinaryDecoderUnitTest.cs) (correction capacity boundaries), [QRCodeDecoderZXingCrossTest](../../../tests/FeatherQR.Tests/StandardQr/QRCodeDecoderZXingCrossTest.cs) (independent-encoder cross-validation), [QRCodeDecoderImageTest](../../../tests/FeatherQR.Tests/StandardQr/QRCodeDecoderImageTest.cs) (rotation/mirror/scale/inversion image cases), [QRCodeDecoderPerspectiveTest](../../../tests/FeatherQR.Tests/StandardQr/QRCodeDecoderPerspectiveTest.cs) (keystone envelope, rotation+perspective combinations), [KeystoneFrameDecodeTest](../../../tests/FeatherQR.Tests/StandardQr/KeystoneFrameDecodeTest.cs) (keystone past what the parallelogram of the finders anchors, synthetic and zxing-cpp's real images, corners within half a module), [BitReaderParityTest](../../../tests/FeatherQR.Tests/Shared/BitReaderParityTest.cs) (the 64-bit window reader against the bit-by-bit definition, shared across symbologies), [ByteSegmentCharsetParityTest](../../../tests/FeatherQR.Tests/Shared/ByteSegmentCharsetParityTest.cs) (byte-segment charset rule over an exhaustive short-sequence corpus, both UTF-8 implementations and the planner mirror), [QRBinaryDecoderTruncationTest](../../../tests/FeatherQR.Tests/StandardQr/QRBinaryDecoderTruncationTest.cs) (every mode cut at every byte).
