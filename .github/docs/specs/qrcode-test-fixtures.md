...
`mode` is `Unknown` where no generator self-reports it: the Structured Append lineages and the third-party captures. The one exception is `Kanji`, which the gate sets when the bytes the set carries are the case text in Shift_JIS and no ECI header is present, a combination only Kanji segments produce.
