...
| - | Binarization (Otsu), shared with Standard QR (lifted to `Internals.ImageDecoders` when Micro QR became the second consumer; whole-buffer histogram, not strided; the same histogram also yields the grey levels of the two classes it separates) | [Binarizer.ComputeOtsuThreshold](../../../src/FeatherQR/Internals/ImageDecoders/Binarizer.cs) |
