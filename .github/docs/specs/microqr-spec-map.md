...
| - | High-level image builder (PNG/JPEG/WEBP/SVG, fluent options; no icon overlay or finder styling, single finder, no ECC headroom); shared options and the output surface come from the common `SymbolImageBuilderBase<TSelf>` | [MicroQRCodeImageBuilder](../../../src/FeatherQR.SkiaSharp/MicroQRCodeImageBuilder.cs), base in [SymbolImageBuilderBase](../../../src/FeatherQR.SkiaSharp/SymbolImageBuilderBase.cs) |
| - | Low-level canvas rendering (module-run merging shared with Standard QR through the internal `IModuleMatrixView` struct views) | [SymbolRenderer.Render (MicroQRCodeData overload)](../../../src/FeatherQR.SkiaSharp/SymbolRenderer.cs), views in [ModuleMatrixView](../../../src/FeatherQR/Internals/ModuleMatrixView.cs) |
