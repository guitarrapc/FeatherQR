...
**Left for later, by design.** The Playground decode panel could draw the outline (a `QrInterop` + JS change, with its own manual verification); a derived `IsMirrored` was not added because the winding already carries it and the public-API rule wants a request first.
