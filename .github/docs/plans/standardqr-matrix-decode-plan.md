...
### 6. Reed-Solomon on ARM64 (phase 5)
...
The syndrome pass is the only cost a clean block pays. It has three tiers (`EccBinaryDecoder.cs`): GFNI on net10.0+ x64, where one instruction is the GF(2^8) multiply; AdvSimd on ARM64, which already multiplies with PMULL and pays for the reduction mod 0x11D itself through two nibble-table lookups; and a scalar log-domain path elsewhere. The first draft of this section said the ARM64 kernel did not use PMULL and proposed it as the hypothesis; reading the kernel refuted that before anything was measured. What its own comments name as the cost is the carried dependency chain, acc → PMULL → UZP → TBL → EOR, once per four bytes, with the observation that halving the vector work had bought only 20 %: a loop bound by latency, not throughput.

The hypothesis that held is re-association. Horner over eight bytes is <c>acc·α^8i ^ (D0·α^4i ^ D1)</c> where D0 and D1 are the existing four-byte data terms; the multiply that joins D0 and D1 depends on data alone and falls off the chain, so eight bytes cost one carried reduce instead of two, for the same multiply count per byte. It needs one new 32-byte constant (α^(8i)) and no length-dependent term, which is what separates it from the earlier four-way chain split the decoder spec records as unshipped: that one won at 149 codewords and lost at rMQR sizes because its combine multiplier depends on the block length.

Two things constrained the work. The 5.4 us came from a harness that timed `TryCorrect` in a loop with a restore copy subtracted out, so phase 5 re-measured it on its own first (5.38 us, direct calls, same blocks). And a destination span shorter than the kernel's store width throws on the GFNI tier but silently corrupts the caller's stack on ARM64, which is why `EccBinaryDecoderKernelParityTest` pins the store width from the kernel side; the new kernel is held to the same pin.
...
| 5 | P1 | Reed-Solomon on ARM64 | Re-measure `EccBinaryDecoder.TryCorrect` on its own over clean blocks; read the AdvSimd syndrome kernel's disassembly and say where the time goes; then one hypothesis a variant, the carried chain first. `DeinterleaveCodewords` only after the syndrome question is settled either way | Done, see Progress log. Five variants gated against the scalar kernel before timing; the eight-byte tree shipped in the shared kernel behind the same tier gate, `EccBinaryDecoderKernelParityTest` extended (the α^(8i) constant, every length mod 8), five planted faults each red; faster or level through `TryCorrect` on every block shape of all three symbologies but Micro QR M1 (+5 ns); end-to-end delta from `QRCodeDecodeEndToEnd` and `QRCodeStructuredAppendDecode` on the M2, before and after |
...
### Phase 5, Reed-Solomon on ARM64 (2026-09-21)

Done: `EccBinaryDecoder.ComputeSyndromesAdvSimd` takes eight bytes a step as a tree, acc·α^8i ^ (D0·α^4i ^ D1), where D0 and D1 are the kernel's existing four-byte data terms; the four-byte step stays as the remainder and the one-byte step as the tail. One 32-byte constant added (`AdvSimdAlpha8`), no table changed, the tier gate and the 32-lane store contract unchanged. Tests first: `AdvSimdConstants_MatchFirstPrinciplesConstruction` was red at compile time on the new constant, and the boundary sweep gained every length mod 8 (eccCount + 1 to + 9) plus 40, 48, 152 and 153. Planted faults, each red against the parity class: the carried multiplier α^8i → α^4i, the D0 multiply dropped, the high group's two terms swapped, the four-byte remainder stepping by α^8i, the high group's carried multiplier read from lanes 0 to 15. Full suite green on net10.0 Release and Debug (12,013 and 12,003 tests; net8.0 has no runtime on this machine). Spec: the ARM syndrome bullet in `standardqr-decoder.md` replaced (it recorded a four-way chain split as an unstarted win), the phase 4 sentence on the packed walk closed, and `qrcode-symbologies.md`'s closed ARM64 queue told it was reopened once and on what basis.

The order the phase row fixed, and what each step gave:

1. Re-measured on its own, direct calls over the 25 clean blocks of a version 40-L symbol: `TryCorrect` 5.38 us, the syndrome pass alone 5.33. The phase 4 figure (5.4) stood.
2. The full-opt disassembly of the four-byte loop: per step, PMULL2 + PMULL on the accumulator, UZP1/UZP2, AND and USHR, two TBL, two EOR, then the three table loads and the broadcast XORed one after another into the accumulator, so the carried chain is about 14 cycles a step and the four data XORs are on it. Two accumulator groups, each with its own chain. Nothing spilled.
3. Re-association, one hypothesis at a time, each gated against the scalar kernel over every ECC count and every length mod 8 before it was timed, all in one run with a byte-identical copy of the candidate as the noise canary (its spread 0.0 to 1.9 %). The eight-byte tree reads 0.44x of the four-byte step at 149-byte blocks with 30 ECC, 0.60x at 45, 0.56x to 0.67x over rMQR's 39- to 58-byte blocks and 0.90x at its smallest 13-byte block; nothing slower anywhere. A sixteen-byte tree ties at 149 bytes and loses 1 to 15 % below it, so from eight bytes on the loop is throughput-bound (its listing is about 105 instructions a step against the four-byte loop's 54, the same density with half the carried reduces), and two throughput hypotheses did not move it: pairing each table row's two half loads through immediate offsets lost 3 to 29 %, and pre-splitting the multiplier halves to remove one register copy per PMULL tied within the canary. The four-way chain split the decoder spec had kept as an unstarted win reads 145.7 ns at 149 bytes against the tree's 95.8 and loses to the four-byte step at every rMQR size, and is closed.
4. `DeinterleaveCodewords` was not opened. It is 2.5 us of what is now a 16.7 us decode, and the phase row made it conditional on the syndrome question, which took the round.

Through `TryCorrect` on a clean block of every shape the three symbologies use (98 Standard QR, 54 rMQR, 8 Micro QR from Table 9), before against after, minimum of two alternating runs of 15 rounds: Standard QR 0.46x to 0.89x, median 0.63x; rMQR 0.45x to 0.98x, median 0.76x; Micro QR 0.76x to 0.81x at M3 and M4, 1.01x at M2's ten-byte block and 1.07x at M1's five-byte block, +5 ns on a call that reads 79 ns on both trees. Version 40-L's 148-byte block 213 → 115 ns, 40-H's 45-byte block 53.6 → 40.6.

Benchmark delta, same harness as phase 4 built against HEAD and against the tree, three alternating rounds, minimum of 15 each, the shipped path (byte run walk behind the vector unpack), us per decode:

| | `QRCodeData` entry, before | after | span entry, before | after |
|---|---:|---:|---:|---:|
| version 1, Byte | 0.28 | 0.26 to 0.27 | 0.26 | 0.24 to 0.25 |
| version 10, Byte | 2.07 to 2.08 | 1.82 to 1.85 | 2.01 | 1.76 to 1.79 |
| version 40, Byte | 19.49 to 19.63 | 16.66 to 16.81 | 19.00 to 19.14 | 16.14 to 16.30 |
| version 40, Numeric | 25.29 to 25.69 | 22.61 to 22.80 | 24.74 to 25.15 | 22.08 to 22.35 |

Small symbols through the string overloads, ns, minimum of three rounds each side: Standard QR version 1 Numeric 258 → 241 to 245, version 1 Alphanumeric 297 → 278 to 282, version 6 URL 931 → 827 to 860; Micro QR Numeric 404 → 395 to 410, Alphanumeric 619 → 612 to 616, Byte 961 → 949 to 958; rMQR Numeric 282 → 263 to 264, Alphanumeric 289 → 270 to 272, Byte 406 → 367 to 368. Nothing slower.

BenchmarkDotNet on the M2, before and after in the same session: `QRCodeDecodeEndToEnd` version 40-L 20.11 → 17.27 us, version 40-L span 19.99 → 16.68, version 40-H span 18.57 → 16.64, version 6 URL 1,004 → 949 ns, version 1 Numeric 312 → 282 ns; `QRCodeStructuredAppendDecode` byte-45k-any 330 → 294 us a set (plain 333 → 283), byte-4k-max10 33.2 → 29.1 (plain 33.3 → 30.2). Allocated unchanged on every row of both classes, the span rows at zero. The mixed, numeric and UTF-8 set rows of the before run carried errors larger than their means and are not quoted; their after rows read 265, 354 and 280 us with errors under 5 %.

Lessons:
- The hypothesis written into this plan before the code was read was wrong, and reading the kernel refuted it in a minute: the ARM64 tier already used PMULL, and its own comment named the real cost. A plan may carry a hypothesis, but not before the file it is about has been opened.
- Re-association is the chain-splitting form that costs nothing at small sizes. The earlier four-way split needed a combine multiplier that depends on the block length, so its fixed cost was the same on a 25-byte rMQR block as on a 149-byte one, and it lost where rMQR lives. The tree's constants are the same for every length, and the only shapes it does not beat are the five- and ten-byte ones, by 5 ns and 0 ns.
- A kernel's number depends on its call site, and only one call site is real. The same shipped kernel read 94 ns a block from one direct loop, 148 from another, 198 through a `Func`, and 99 through `TryCorrect`; the old kernel read the same everywhere, and no cause was pinned down (nothing spills, nothing allocates). Ratios were read only between candidates on one call site, and the shipped figures are the `TryCorrect` ones. A standalone sweep that read the tree as slower at 20 to 28 bytes was that artifact, and the `TryCorrect` sweep at the same shapes read 0.85x to 0.93x.
- Throughput was the wall two steps in. The first tree halved the chain and paid 2.2x; the second tree, which should have quartered it, paid nothing, and both throughput hypotheses lost or tied. When a re-association stops paying, the next instruction removed has to be a real one, not a copy the JIT would have hidden anyway.
- The mutation run started from a green baseline this time, with the count checked (6 passed, 2 GFNI skips) before the first fault went in.

From the start of this plan a version 40 Byte symbol has gone 170 → 43 → 15 (x64) and, on the M2, 167 to 186 → 19.5 → 16.7 us. Extraction is now 10.2 of 16.7 us, 61 % of the decode; deinterleave 2.5, the syndrome pass 2.5, the rest under 1.5.
